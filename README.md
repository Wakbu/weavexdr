...
일반 사용자는 릴리스 ZIP을 내려받아 압축을 풀고 `WeaveXDR.exe`만 실행한다. API 토큰을 만들거나 입력할 필요가 없다. 브라우저가 자동으로 열리지 않으면 EXE를 종료한 뒤 다시 실행하며, 주소창에서 `127.0.0.1:8765/dashboard`를 먼저 직접 열지 않는다.

EXE는 실행할 때마다 임시 인증 정보를 자동 생성해 브라우저 보안 세션으로 바꾼다. 주소에 잠깐 보이는 `#token=...` 값은 자동 연결용이며 복사·저장·재사용하지 않는다. 설정의 `고정 API 토큰`은 개발자가 외부 API 프로그램을 직접 연결할 때만 쓰는 고급 기능이다. 자세한 복구 절차는 [운영·복구 안내](docs/OPERATIONS.md#일반-사용자의-인증-절차)를 참고한다.
...
현재 회귀 테스트는 `125 passed`, 기준 평가 데이터는 `30/30 passed`다.
