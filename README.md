...
- dry-run 결과는 항상 `executed=False`이며 실제 실행기와 명확히 분리

## 실제 대응 실행기

`ActualResponseService`는 dry-run과 명령별 승인을 다시 확인한 뒤 제한된 실제 대응을 실행한다.

- 프로세스 종료 직전 PID·시작 시각·이미지 경로 재조회
- 파일 격리 직전 SHA-256 재계산, 전용 저장소 이동과 SQLite 복원 메타데이터
- 복원 시 격리본 무결성 및 원래 경로 충돌 검사
- Windows 방화벽 아웃바운드 IP 차단과 규칙 ID 기반 해제
- 최대 2회 제한 재시도 후 자동 반복을 멈추고 수동 검토로 전환
- 분석·대응·복구 결과를 SHA-256 연결 구조의 SQLite 감사 로그에 기록
