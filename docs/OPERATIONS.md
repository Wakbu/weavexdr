...
지원 기준은 Windows 11이다. 포터블 `WeaveXDR.exe` 사용자는 Python 설치, API 토큰 발급 또는 환경 변수 설정이 필요 없다. 아래 환경 변수 등록은 Windows 서비스 설치를 선택한 운영자에게만 해당하며, 실제 값은 문서·설정·명령 기록에 남기지 않는다.
...
1. 열려 있는 WeaveXDR 화면에서 왼쪽 아래 `WeaveXDR 종료`를 눌러 기존 서버를 먼저 닫는다.
2. 최신 릴리스 ZIP의 압축을 푼 뒤 폴더 안의 `WeaveXDR.exe`를 한 번 실행한다.
3. 자동으로 열린 브라우저를 사용한다. 인증 정보는 EXE와 브라우저가 내부에서 자동 교환한다.
4. 연결이 끝나면 `API 연결됨`이 표시된다.
5. `자동 연결을 완료하지 못했습니다` 창이 계속 나오면 기존 EXE와 브라우저 탭을 모두 종료한 다음 최신 EXE를 다시 실행한다.

설정 화면에는 인증 입력란이 없다. curl, PowerShell 또는 별도 연동 프로그램이 API를 직접 호출하는 고급 사용만 환경 변수 방식의 고정 토큰을 사용한다. 서버가 꺼진 상태나 다른 프로세스가 8765를 사용 중인 경우에도 즐겨찾기 주소를 직접 열지 말고 EXE를 실행한다. EXE가 사용 가능한 loopback 포트를 선택해 올바른 브라우저 주소를 자동으로 연다.
...
종료 요청은 이벤트 스트림, Sysmon 수집기와 DB를 순서대로 닫는다. 정상 정리가 비정상적으로 멈춘 경우에도 종료 감시기가 20초 후 해당 WeaveXDR 런타임을 끝내므로 사용자가 작업 관리자에서 프로세스를 별도로 찾을 필요가 없다.
