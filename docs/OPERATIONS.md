...
WeaveXDR는 사용자별 단일 인스턴스로 실행된다. EXE를 다시 실행하면 PID·포트·버전을 재검증한 뒤 기존 대시보드를 연다. 트레이에서 대시보드 열기, Sysmon 수집 일시정지·재개와 정상 종료를 수행할 수 있다.

설정 화면은 앱 버전·빌드 날짜와 Windows 시작 프로그램 상태를 표시한다. 자동 시작은 기본적으로 꺼져 있으며 사용자가 설정 또는 설치 마법사에서 직접 선택한 경우에만 등록된다. `install_wizard.ps1`은 Sysmon·재부팅 대기 상태를 먼저 보여주고 Program Files·서비스 등록이 필요한 실제 설치 단계에서만 UAC를 요청한다.
...
- **사건**: 검색과 판정 필터로 조사할 사건을 선택한다. 목록은 서버에서 50건 또는 100건 단위로 불러오며 상단 전체 사건 수는 현재 페이지와 무관한 정확한 저장 건수다. 초기에는 `안전한 데모 생성`으로 화면 흐름을 확인할 수 있으며 실제 악성 코드를 실행하지 않는다.
- **조사**: 요약, 원본 증거, 시간순 공격 흐름과 권장 대응을 한 사건 단위로 확인한다.
- **설정**: Sysmon 수집 상태, 처리 이벤트 수, 마지막 수집 시각, 필요한 권한 조치와 API 세션 상태를 확인한다.
- **대응 안전장치**: 실제 대응 활성 여부, 프로세스 트리 검사, 복구 가능 조치와 플레이북 사용 여부를 확인한다.
- **데이터 저장소**: DB 무결성·크기·디스크 여유·보존 기간·스키마 버전을 확인하고 사용자 확인 후 검증된 백업을 만든다.
- **빠른 파일 검사**: `파일 선택` 또는 `폴더 선택`으로 Windows 기본 선택창을 열어 범위를 고른다. 경로를 고르지 않으면 기본 범위를 사용한다. 최근 30일의 실행·스크립트·압축·Office 위험 형식을 우선 처리하며 파일 목록 계산, 정적 분석과 Defender 마무리 단계를 구분하고 파일/초·예상 잔여 시간·캐시 사용량을 표시한다.
...
`install.ps1`은 `%ProgramFiles%\WeaveXDR`에 전용 가상환경과 wheel을 설치하고 `WeaveXDR` Windows 서비스를 수동 시작으로 등록한다. 설치만으로 서비스나 데스크톱 앱이 실행되지는 않는다. 서비스는 사용자가 명시적으로 시작했을 때 일반 분석·저장·loopback API를 제공한다. 방화벽 대응 등 관리자 조치는 기존 승인 및 재검증 정책을 별도로 통과해야 한다.
...
실제 대응은 일반 실행에서 꺼져 있다. 격리·프로세스 종료·방화벽 차단을 시험해야 하는 관리 환경에서만 프로세스 시작 전에 `WEAVEXDR_ENABLE_ACTIVE_RESPONSE=1`을 설정한다. 각 명령은 여전히 dry-run, 영향 미리 보기, 대상 재검증과 개별 승인을 통과해야 한다. 기간형 네트워크 차단은 런타임이 주기적으로 만료를 확인하며 복구 기록은 DB에 유지된다.

설정의 `백업 만들기`는 무결성을 검사한 SQLite 사본을 만들고, `만료 자료 보관`은 보존 기간이 지난 자료를 삭제하지 않은 채 gzip JSON으로 내보낸다. 복원은 정상 백업 파일명을 다시 입력해야 예약되며, 실행 중 DB를 교체하지 않고 WeaveXDR를 정상 종료한 다음 일반 실행할 때 적용된다. 적용 전 DB는 `.rollback` 사본으로 남는다.
...
- 비정상 종료 흔적이 있으면 다음 실행에서 DB 연결 전에 무결성을 확인하고 검증 백업을 만든다. 손상 시 원본을 `.corrupt-시각.db`로 보존한 뒤 검증된 롤백만 자동 적용한다.
- 설정의 실행 자원에서 전원 모드·배터리·절전 복귀 감지와 비정상 종료 복구 결과를 확인한다. 배터리 30% 이하에서는 파일 폴링만 8초로 완화되고 실시간 이벤트 수집은 유지된다.
...
- 파일 검사 화면의 Windows 기본 선택창에서 파일·폴더를 고른 뒤 빠른·전체·사용자 지정 검사를 실행하고 진행률·취소를 관리한다. 경로 문자열은 직접 입력하지 않는다.
- 파일·폴더 선택창은 브라우저가 아닌 Windows 기본 대화상자이며 항상 대시보드 앞에 표시된다. 선택을 취소하면 기존 목록을 유지한다.
- 격리 복원은 반드시 `재검사 후 복원` 흐름과 사용자 확인을 거친다.
- Sigma는 가져온 직후 비활성 상태다. MITRE/CISA 후보는 JSON·SHA-256·회귀 기준을
  통과한 뒤 활성화하며 문제 발생 시 직전본으로 롤백한다.
- 외부 평판 어댑터는 명시적 동의, SHA-256 전용 입력과 공급자 요금 제한을 모두
  만족해야 호출된다.
