...
3. 자동으로 열린 브라우저를 사용한다. 주소에 잠깐 나타나는 `#token=...`은 자동 연결용이므로 복사하거나 따로 보관하지 않는다.
4. 연결이 끝나면 주소에서 토큰 부분이 자동으로 사라지고 `API 연결됨`이 표시된다.
5. `로컬 API 다시 연결` 창이 계속 나오면 아무 토큰이나 만들지 말고 창을 닫는다. 기존 EXE와 브라우저 탭을 모두 종료한 다음 최신 EXE를 다시 실행한다.

설정 화면의 `고정 API 토큰` 입력란은 curl, PowerShell 또는 별도 연동 프로그램이 API를 직접 호출할 때만 사용한다. 대시보드만 사용하는 일반 사용자는 비워 둔다. 브라우저 즐겨찾기는 인증 정보가 제거된 `http://127.0.0.1:8765/dashboard`로 저장할 수 있지만, 서버가 꺼진 상태에서는 즐겨찾기 대신 EXE를 먼저 실행해야 한다.
